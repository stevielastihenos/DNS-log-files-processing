...
blockingfile = "dnslog_with_blocking.txt"
            ...
            try:
                ...
                if op == 0:
                    ...
                    if len(qs) > 0:
                        ...
                        qdomain = str(q).split()[0]
                        if "ads" in qdomain or "click" in qdomain or "track" in qdomain or "adv" in qdomain:
                            spoof[qdomain] = "192.168.127.127"
                        else:
                            save_req(blockingfile, 'Client IP: ' + address[0] + '    request is    ' + str(q) + '\n')

                        save_req(LOGREQFILE, 'Client IP: ' + address[0] + '    request is    ' + str(q) + '\n')
                        DEBUGLOG('request is ' + str(q))

			
                        # if str(q) != "192.168.127.127":
                        #     save_req(LOGREQFILE, 'Client IP: ' + address[0] + '    request is    ' + str(q) + '\n')
                        # else:
                        #     save_req(blockingfile, 'Client IP: ' + address[0] + '    request is    ' + str(q) + '\n')
